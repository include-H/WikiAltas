 ...
 *
 * 媒体库指令（"把 X 挂到 Y""关联上"）也是派活：动的是结构不是正文，
 * 但同样需要写工具面（attach_library_link），判成聊天会让它拿不到工具。
 */
const WRITE_VERBS =
  /(写(?!得|的)|新建|建个|建立|建档|创建|新增|补(?!丁)|扩写|扩充|改写|重写|整理|完善|润色|修订|继续|接着|生成|做一?篇|来一?篇|起稿|落笔|加一?段|加一?章|调整|改|挂到|挂上|挂进|关联到|关联上|绑到)/
